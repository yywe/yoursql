...
impl PhysicalPlanner for DefaultPhysicalPlanner {
    async fn create_physical_plan(&self, logical_plan: &LogicalPlan, session_state: &SessionState) -> Result<Arc<dyn ExecutionPlan>>{
        let plan = self.create_initial_plan(logical_plan, session_state).await?;
        Ok(plan)
    }
...
impl DefaultPhysicalPlanner {
    pub fn physical_optimize(&self, plan: Arc<dyn ExecutionPlan>, _session_state: &SessionState) ->  Result<Arc<dyn ExecutionPlan>> {

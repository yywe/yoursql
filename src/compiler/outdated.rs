...
use anyhow::{anyhow};
use super::buffering_consumer::BufferingConsumer;
...
pub struct LLVMValue<'ctx> {
    ...
    pub size: Option<IntValue<'ctx>>, /* the size of the value (for string, include the null
                                      * terminator) */
...
pub struct Pipelineold<'ctx> {
    pub operators: Vec<Box<dyn OperatorTranslatorold<'ctx>>>,
}

pub trait OperatorTranslatorold<'ctx> {
...
impl ResultCollector {
    ...
    pub fn get_result(&self) -> Result<RecordBatch> {
        ...
        for ptr in self.buffer.iter() {
            ...
            let data: &[u8] =
                unsafe { std::slice::from_raw_parts(ans_ptr as *const u8, len as usize) };

            let batch = deserialize_batch(schema.clone(), data)?;

            // unsafe {
            //    libc::free(*ptr as *mut libc::c_void); //note this requires the memory allocated
            // using malloc
            //}
...
impl<'ctx> CodeGen<'ctx> {
    ...
    pub fn compile_plan<'a>(
    ...
    ) -> Result<JitFunction<QueryPlan>> {
        // for now define a test function just to test the LLVM
        // create a function which takes a pointer of table_name and database_name and session_state
        // pointer and return a pointer of RecordBatch
        let fn_type = self.types.ptrtype.fn_type(
            &[
        ...
        let call_result = self
            ...
            .build_call(
                loadtbl_func,
                &[table_name.into(), database_name.into(), session.into()],
                "",
            )
            .unwrap();
        let result = self
        ...
        self.builder
            .build_store(ti, self.types.i32type.const_zero())?;

        let new_result_ptr = self.types.ptrtype.const_null();

        // create a loop. TODO: here needs function, we should have a better way to associate
        // function with builder.
        let cur_function = self
            .builder
            .get_insert_block()
            .unwrap()
            .get_parent()
        ...
        let ti_val = self
            .builder
            .build_load(self.types.i32type, ti, "")
            .unwrap()
            .into_int_value();
        let tcond = self
            .builder
            ...
            .unwrap();
        self.builder
            .build_conditional_branch(tcond, tloop_body, tloop_end)?;
        ...
        let row_ptr = self
            .builder
            .build_load(self.types.ptrtype, row_ptr, "")?
            .into_pointer_value();
        ...
        // seralize/deseralize a batch of multiple rows using llvm may be complex as we need to
        // maintain the buffer size and the offset of each row.
        ...
        // the idea here is that we allocate a new memory block in heap, and serialize the row data
        ...
        for value in row.values.iter() {
            let value_size = match value.data_type {
                ...
                DataType::Utf8 => {
                    ...
                    let field_size = self.builder.build_int_add(
                        self.types.i32type.const_int(4, false),
                        size,
                        "",
                    )?;
                    field_size
            ...
            row_size = self
                .builder
                .build_int_add(row_size, value_size, "row_size")?;
        }
        row_size = self.builder.build_int_add(
            row_size,
            self.types.i32type.const_int(row.values.len() as u64, false),
            "",
        )?; // the null bitmap
            // allocate the memory block
            // let new_result = self.builder.build_array_malloc(self.types.i8type, row_size,
            // "new_result")?;

        // serialize the row data
        // first write the total size of the row data. now equals = 4 + 4 + 4 + row_size (remember
        // total_size:number_of_rows:row_offset:row_data)
        let total_size = self.builder.build_int_add(
            self.types.i32type.const_int(12, false),
            row_size,
            "total_size",
        )?;
        ...
        self.builder.build_call(
            ...
            &[
                new_result.into(),
                self.types.i32type.const_int(0, false).into(),
                total_size.into(),
            ],
            "",
        )?;
        ...
        self.emit_printf_call(
            "$$$$$the address of new_result is-----> %p\n",
            &[new_result.into()],
        );

        self.emit_printf_call(
            "$$$$the total size of the row data----->: %d\n",
            &[total_size.into()],
        );
        let total_size_ptr = new_result.const_cast(self.types.ptrtype);
        self.builder.build_store(total_size_ptr, total_size)?;

        // read it out and check.
        let total_sizetemp = self
            .builder
            .build_load(self.types.i32type, total_size_ptr, "")?
            .into_int_value();
        self.emit_printf_call(
            "$$$$verified total size of the row data----->: %d\n",
            &[total_sizetemp.into()],
        );

        // write the number of rows
        let num_rows_ptr = unsafe {
            total_size_ptr.const_in_bounds_gep(
                self.types.i32type,
                &[self.types.i32type.const_int(1, false)],
            )
        };
        self.builder
            .build_store(num_rows_ptr, self.types.i32type.const_int(1, false))?;
        // write the row offset
        let row_offset_ptr = unsafe {
            num_rows_ptr.const_in_bounds_gep(
                self.types.i32type,
                &[self.types.i32type.const_int(1, false)],
            )
        };
        self.builder
            .build_store(row_offset_ptr, self.types.i32type.const_int(12, false))?;
        // write the row data
        let mut row_data_ptr = unsafe {
            new_result.const_in_bounds_gep(
                self.types.i8type,
                &[self.types.i32type.const_int(12, false)],
            )
        };
        ...
        for value in row.values.iter() {
            ...
            match value.data_type {
                DataType::Int32 => {
                    let field_ptr = row_data_ptr.const_cast(self.types.ptrtype);
                    self.builder
                        .build_store(field_ptr, value.value.into_int_value())?;
                    row_data_ptr = unsafe {
                        row_data_ptr.const_in_bounds_gep(
                            self.types.i8type,
                            &[self.types.i32type.const_int(4, false)],
                        )
                    };
                ...
                DataType::Int64 => {
                    ...
                    row_data_ptr = unsafe {
                        row_data_ptr.const_in_bounds_gep(
                            self.types.i8type,
                            &[self.types.i32type.const_int(8, false)],
                        )
                    };
                }
                DataType::Float32 => {
                    let field_ptr = row_data_ptr.const_cast(self.types.ptrtype);
                    self.builder
                        .build_store(field_ptr, value.value.into_float_value())?;
                    row_data_ptr = unsafe {
                        row_data_ptr.const_in_bounds_gep(
                            self.types.i8type,
                            &[self.types.i32type.const_int(4, false)],
                ...
                DataType::Utf8 => {
                    ...
                    let field_size_ptr = row_data_ptr.const_cast(self.types.ptrtype);
                    self.emit_printf_call(
                        "$$$$$store size %d to addrss:%p\n",
                    ...
                    self.emit_printf_call(
                        "$$$store string to addrss:%p from %p\n",
                        &[field_ptr.into(), value.value.into()],
                    );

                    self.emit_printf_call(
                        "$$$$$$$$try to copy string:%s, size=%d\n",
                    ...
                    // self.builder.build_call(self.module.get_function("strcpy").unwrap(),
                    // &[field_ptr.into(), value.value.into()], "")?;

                    // let tchar = self.types.i8type.const_int(10, false);
                    // self.builder.build_store(field_ptr, tchar)?;
                    // let field_ptr = unsafe {self.builder.build_in_bounds_gep(self.types.i8type,
                    // field_ptr, &[self.types.i32type.const_int(1, false)], "")?};
                    // self.builder.build_store(field_ptr,  tchar)?;
                    // let field_ptr = unsafe {self.builder.build_in_bounds_gep(self.types.i8type,
                    // field_ptr, &[self.types.i32type.const_int(1, false)], "")?};
                    // self.builder.build_store(field_ptr,  tchar)?;
                    ...
                    // note the size of the string is known at runtime, we need to build gep to the
                    // next field
                ...
                DataType::Binary => {
                    ...
                    // note the size of the binary is also is known at runtime, we need to build gep
                    // to the next field
                    row_data_ptr = unsafe {
                        self.builder.build_in_bounds_gep(
                            self.types.i8type,
                ...
                DataType::Int16 => {
                    ...
                    row_data_ptr = unsafe {
                        row_data_ptr.const_in_bounds_gep(
                            self.types.i8type,
                            &[self.types.i32type.const_int(2, false)],
                        )
                    };
                }
                DataType::Int8 => {
        ...
        for value in row.values.iter() {
            let null_bitmap_ptr = row_data_ptr.const_cast(self.types.ptrtype);
        ...
        self.emit_printf_call(
            "$$$$$the address of row_data_ptr is %p\n",
            &[row_data_ptr.into()],
        );
        ...
        let ptr_addr_llvm = self
            .builder
            .build_int_to_ptr(ptr_addr, self.types.ptrtype, "")?;
        self.builder.build_call(
            deliver_row_func,
            &[new_result.into(), ptr_addr_llvm.into()],
            "",
        )?;

        // read it out and check.
        let total_sizetemp2 = self
            .builder
            .build_load(self.types.i32type, total_size_ptr, "")?
            .into_int_value();
        self.emit_printf_call(
        ...
        let next_ti = self.builder.build_int_add(
            ti_val,
            self.types.i32type.const_int(1, false),
            "next_ti",
        )?;
        ...
        let ret = self.types.ptrtype.const_null();
     ...
     pub fn load_data_row<'a, 'b:'a>(&'a self, row_ptr: PointerValue<'b>, schema: Arc<Schema>) -> Result<LLVMRow> {
        ...
        for field in schema.fields.iter() {
            ...
            match dtype {
                DataType::Int32 => {
                    let field_ptr = cur_addr.const_cast(self.types.ptrtype);
                ...
                DataType::Utf8 => {
                    let field_size_ptr = cur_addr.const_cast(self.types.ptrtype);
    ...
    pub fn load_batch_meta(&self, dataptr: PointerValue<'ctx>) -> Result<LLVMRowBatch> {
        let sizeptr = dataptr.const_cast(self.types.ptrtype);
        let size = self
            .builder
            .build_load(self.types.i32type, sizeptr, "")?
            .into_int_value();
        let rownumptr = unsafe {
            sizeptr.const_in_bounds_gep(
                self.types.i32type,
                &[self.types.i32type.const_int(1, false)],
            )
        };
        let num_rows = self
            .builder
            .build_load(self.types.i32type, rownumptr, "")?
            .into_int_value();
        // allocate the row pointer array in heap. TODO: verify when to free the memory
        // note LLVM also support alloca array in stack, like VLA (variable length array) in C. but
        // there are some risks of stack overflow note the type is ptrtype, as the row
        // pointer is pointer to row address.
        let row_ptrs = self
            .builder
            .build_array_malloc(self.types.ptrtype, num_rows, "")?;
        ...
        self.builder
            .build_store(i, self.types.i32type.const_zero())?;
        ...
        let i_val = self
            .builder
            .build_load(self.types.i32type, i, "")
            .unwrap()
            .into_int_value();
        let cond = self
            .builder
            .build_int_compare(inkwell::IntPredicate::ULT, i_val, num_rows, "loop_cond")
            .unwrap();
        self.builder
            .build_conditional_branch(cond, loop_body, loop_end)?;

        self.builder.position_at_end(loop_body);
        // note there are two offsets. one is the offset of the row pointer array, the other is the
        // offset of the row data calculate offset location. ith row, its offset is at:
        // dataptr + 4 + 4 + i*4
        let offset_arr_idx = self
            .builder
            .build_int_mul(i_val, self.types.i32type.const_int(4, false), "")
            .unwrap();
        let offset_arr_idx = self
            .builder
            .build_int_add(offset_arr_idx, self.types.i32type.const_int(8, false), "")
            .unwrap();
        self.emit_printf_call("the offset array: %d\n", &[offset_arr_idx.into()]);
        let offset_ptr = unsafe {
            self.builder
                .build_in_bounds_gep(self.types.i8type, dataptr, &[offset_arr_idx], "")?
        };

        // now load the offset value of the row data from the start of the memory block
        let offset_value = self
            .builder
            .build_load(self.types.i32type, offset_ptr, "")?
            .into_int_value();
        ...
        let row_ptr = unsafe {
            self.builder
                .build_in_bounds_gep(self.types.i8type, dataptr, &[offset_value], "")?
        };
        ...
        let row_ptr_addr = unsafe {
            self.builder
                .build_in_bounds_gep(self.types.ptrtype, row_ptrs, &[i_val], "")?
        };
        self.builder.build_store(row_ptr_addr, row_ptr)?;

        //--- Test code. Test if the row_ptr_addr is correct
        self.emit_printf_call(
            "the address of the row_ptr_addr: %p\n",
            &[row_ptr_addr.into()],
        );
        // cast the row_ptr_addr to 64 bit pointer as the column is int
        // let row_ptr_addr =
        // row_ptr_addr.const_cast(self.types.i64type.ptr_type(AddressSpace::default()));
        // load the value at row_ptr_addr
        let row_ptr_value = self
            .builder
            .build_load(self.types.ptrtype, row_ptr_addr, "")?
            .into_pointer_value();
        // print
        self.emit_printf_call(
            "verify the value of the row_ptr_addr: %p\n",
            &[row_ptr_value.into()],
        );
        // load the 1st value, which is the id (int64)
        let row_ptr_value =
            row_ptr_value.const_cast(self.types.i64type.ptr_type(AddressSpace::default()));
        let row_ptr_value = self
            .builder
            .build_load(self.types.i64type, row_ptr_value, "")?
            .into_int_value();
        // print
        self.emit_printf_call(
            "verify the value of at row_ptr_addr: %d\n",
            &[row_ptr_value.into()],
        );
        //--- Test code end
        ...
        let next_i =
            self.builder
                .build_int_add(i_val, self.types.i32type.const_int(1, false), "next_i")?;
    ...
    pub fn deserialize_batch_test(&self, memaddr: PointerValue<'ctx>) -> Result<()> {
        ...
        let row_start = unsafe {
            memaddr
                .const_in_bounds_gep(self.types.i8type, &[self.types.i32type.const_int(1, false)])
        };
        ...
        let u32val = self
            .builder
            .build_load(self.types.i32type, u32ptr, "")?
            .into_int_value();
        ...
        // let rows_numptr = unsafe {memaddr.const_gep(memaddr.get_type(),
        // &[self.types.i32type.const_int(4, false)])}; let rows_numptr =
        // rows_numptr.const_cast(self.types.i32type.ptr_type(AddressSpace::default()));
        let rows_numptr = unsafe {
            u32ptr.const_in_bounds_gep(
                self.types.i32type,
                &[self.types.i32type.const_int(1, false)],
            )
        };
        let rows_num = self
            .builder
            .build_load(self.types.i32type, rows_numptr, "")?
            .into_int_value();
        // print the number of rows
        self.emit_printf_call(
            "pointer location of rows_numptr: %p\n",
            &[rows_numptr.into()],
        );
        self.emit_printf_call("number of rows got in Llvm ir: %d\n", &[rows_num.into()]);

        // simple test can we allocate array dynamic in stack?
        let stack_array = self
            .builder
            .build_array_alloca(self.types.i32type, rows_num, "stack_array")
            .unwrap();

        // now get the 1st row offset.
        let row1_offset_ptr = unsafe {
            u32ptr.const_in_bounds_gep(
                self.types.i32type,
                &[self.types.i32type.const_int(2, false)],
            )
        };
        let row1_offset = self
            .builder
            .build_load(self.types.i32type, row1_offset_ptr, "")?
            .into_int_value();
        // print the 1st row offset
        self.emit_printf_call(
            "pointer location of row1_offset_ptr: %p\n",
        ...
        // now get the 1st row offset. (note this is a temp solution, should add offset to the
        // start, here just use 4)
        let row1e1_offset_ptr = unsafe {
            u32ptr.const_in_bounds_gep(
                self.types.i32type,
                &[self.types.i32type.const_int(4, false)],
            )
        ...
        let row1e1_offset_ptr =
            row1e1_offset_ptr.const_cast(self.types.i64type.ptr_type(AddressSpace::default()));

        let row1e1 = self
            .builder
            .build_load(self.types.i32type, row1e1_offset_ptr, "")?
            .into_int_value();
        // print the 1st row offset
        self.emit_printf_call(
            "pointer location of row1e1_offset_ptr: %p\n",
        ...
        // now get the 1st row offset. (note this is a temp solution, should add offset to the
        // start, here just use 4)
        let row1e2_offset_ptr = unsafe {
            u32ptr.const_in_bounds_gep(
                self.types.i32type,
                &[self.types.i32type.const_int(6, false)],
            )
        };
        // cast to back to i32 type for str size first
        let row1e2size =
            row1e2_offset_ptr.const_cast(self.types.i32type.ptr_type(AddressSpace::default()));

        let row1e1 = self
            .builder
            .build_load(self.types.i32type, row1e2size, "")?
            .into_int_value();
        // print the 1st row offset
        ...
        let row1e2_offset_ptr = unsafe {
            u32ptr.const_in_bounds_gep(
                self.types.i32type,
                &[self.types.i32type.const_int(7, false)],
            )
        };
        // cast to i8ptr type
        let row1e2_offset_ptr =
            row1e2_offset_ptr.const_cast(self.types.ptrtype.ptr_type(AddressSpace::default()));
        // print the string
        self.emit_printf_call(
        ...
        // let row1init =
        // memaddr.const_cast(self.types.ptrtype.ptr_type(AddressSpace::default()));
        // self.emit_printf_call("===>>>the address of the start of the memory: %p\n",
        // &[row1init.into()]); self.emit_printf_call("the offset used is %d\n",
        // &[row1_offset.into()]);
        ...
        let row1e1 = self
            .builder
            .build_load(self.types.i64type, row_start, "")?
            .into_int_value();
        // print the 1st row offset
        self.emit_printf_call(
            "newnewnew===>>>pointer location of row1e1_offset_ptr: %p\n",
            &[row_start.into()],
        );
        ...
        let row2_offset_ptr = unsafe {
            u32ptr.const_in_bounds_gep(
                self.types.i32type,
                &[self.types.i32type.const_int(3, false)],
            )
        };
        let row2_offset = self
            .builder
            .build_load(self.types.i32type, row2_offset_ptr, "")?
            .into_int_value();
        // print the 1st row offset
        self.emit_printf_call(
            "pointer location of row2_offset_ptr: %p\n",
    ...
    pub fn execute(
        ...
        let ans_ptr = unsafe {
            func.call(
                ...
                database_name.as_ptr() as *const i8,
                raw_session_ptr,
            )
        };
        // get the schema and then deserialize the RecordBatch
        let schema = session
            .database(database_name.to_str().unwrap())
            .unwrap()
            .get_table_sync(table_name.to_str().unwrap())
            .unwrap()
            .get_table();
        ...
        // the 1st 4 bytes are the length of the buffer, note it is in little endian and serialized
        // in u32
        ...
        // let data: &[u8] = unsafe { std::slice::from_raw_parts(ans_ptr  as *const u8, len as
        // usize) };
        ...
        //};
        // free the memory passed in by the LLVM (which was allocated by amalloc call in
        // load_table_data function) unsafe {
        //   libc::free(ans_ptr as *mut libc::c_void); //note this requires the memory allocated
        // using malloc
        //}
        // ret
    ...
    // TODO: delete below two functions as they are initial attempt and should not be needed;
    pub fn register_external_func(&self) {
        // Regiser for load_table_data function
        let load_table_type = self.types.ptrtype.fn_type(
            &[
                self.types.ptrtype.into(),
                self.types.ptrtype.into(),
                self.types.ptrtype.into(),
        ...
        let load_table_func = self
            .module
            .add_function("load_table_data", load_table_type, None);
        self.engine
            .add_global_mapping(&load_table_func, load_table_data as usize);

        // Register for deliver_row
        let deliver_row_type = self.types.voidtype.fn_type(
            &[self.types.ptrtype.into(), self.types.ptrtype.into()],
            false,
        );
        let deliver_row_func = self
            .module
            .add_function("deliver_row", deliver_row_type, None);
        self.engine
            .add_global_mapping(&deliver_row_func, deliver_row as usize);
    }


}
///TODO: delete we should not need this, as long as we use primitives types;
/// 
...
mod tests {
    ...
    fn test_serialize_batch() {
     ...
     // RUST_LOG=info cargo test --package yoursql --lib -- compiler::tests::test_llvm --exact
    // --nocapture
    #[test]
    #[ignore]
    fn test_llvm() -> Result<()> {
        ...
        let compiler = CodeGen::new(&context, "test");
        compiler.initialize();
        compiler.register_external_func(); // TODO: delete this test;
        let rt = Runtime::new().unwrap();
        rt.block_on(async {
            session
                .state
                .read()
                .run("create table tbl(id int, name varchar(20))")
                .await
                .unwrap();
            session
                .state
                .read()
                .run("insert into tbl values (2, 'worl'), (65539, 'big')")
                .await
                .unwrap();
        });

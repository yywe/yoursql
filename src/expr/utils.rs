...
pub fn expand_qualified_wildcard(qualifier: &str, schema: &Schema) -> Result<Vec<Expr>> {
    ...
    let qualified_fields: Vec<Field> = schema
...
pub fn from_plan(plan: &LogicalPlan, expr: &[Expr], inputs: &[LogicalPlan]) -> Result<LogicalPlan> {
    match plan {
        ...
        LogicalPlan::Limit(Limit { skip, fetch, .. }) => {
            Ok(LogicalPlan::Limit(super::logical_plan::Limit {
                skip: *skip,
                fetch: *fetch,
                input: Arc::new(inputs[0].clone()),
            }))
        }
        LogicalPlan::TableScan(ts) => Ok(LogicalPlan::TableScan(TableScan {
            filters: expr.to_vec(),
            ..ts.clone()
        })),

use super::Storage;
use super::{Batch, DbMeta, IndexValue, Row, Table, Value};
...
impl SledStore {
    ...
    pub fn validate_row(&self, table: &Table, row: &Row, is_updaterow: Option<u64>) -> Result<()> {
        if row.len() != table.columns.len() {
            return Err(
                StorageError::RowError(table.name.clone(), "length does not match".into()).into(),
            );
        }
        for (index, (column, value)) in table.columns.iter().zip(row.iter()).enumerate() {
            ...
            match value.datatype() {
                ...
                None => {
                    return Err(StorageError::RowError(
                        table.name.clone(),
                        "column cannot be null".into(),
                    )
                    .into())
                }
                Some(datatype) => {
                ...
                match value {
                    ...
                    _ => {
                        ...
                        if fkvalue.is_none() {
                            return Err(StorageError::RowError(
                                table.name.clone(),
                                "refered primary key does not exists".into(),
                            )
                            .into());
                        }
                    }
                ...
                if let Some(row2udpate) = is_updaterow {
                    ...
                    match oldrow {
                        Some(row) => {
                            ...
                            if oldpk != *value {
                                let pkindex =
                                    self.read_indices(table.name.as_ref(), &column.name, value)?;
                                if pkindex.is_some() {
                                    return Err(StorageError::RowError(
                                        table.name.clone(),
                                        "primary key already exists".into(),
                                    )
                                    .into());
                                }
                            }
                ...
                while let Some((k, v)) = records.next().transpose()? {
                    ...
                    if existing.get(index).unwrap_or(&Value::Null) == value {
                        if let Some(update_rowid) = is_updaterow {
                            if k.as_ref()
                                != format!("table/{}/{}", table.name, update_rowid).as_bytes()
                            {
                                return Err(StorageError::RowError(
                                    table.name.clone(),
                                    "column value not unique".into(),
                                )
                                .into());
                            }
        ...
        match self.db.get(&key)? {
            ...
            None => {
                return Err(StorageError::InvalidTableName(
                    name.into(),
                    String::from("table does not exists"),
                )
                .into())
            }
        }
...
impl Storage for SledStore {
    ...
    async fn create_table(&self, table: &Table) -> Result<()> {
        if table.name == "catalog" {
            return Err(StorageError::InvalidTableName(
                table.name.clone(),
                String::from("catalog cannot be table name"),
            )
            .into());
        }
        ...
        if self.db.get(&key)?.is_some() {
            return Err(StorageError::InvalidTableName(
                table.name.clone(),
                String::from("table name alreay exists"),
            )
            .into());
        }
    ...
    async fn update_row(&self, table: &str, id: u64, row: Row) -> Result<()> {
        ...
        let rowkey = format!("table/{}/{}", table, id);
        let oldrow = self
            .read_row_inner(table, id)?
            .context("row does not exist")?;
...
mod test {
    ...
    /*
        #[test]
        fn test_setuptrace() -> Result<()> {
            // this test just setup the tracer, does not do anything. can only be executed once
    ...
    */
    ...
    async fn test_sledstore() -> Result<()> {
        ...
        let updaterow: Row = vec![
            Value::String("c11".into()),
            Value::String("c22".into()),
            Value::String("c33".into()),
        ];
        ss.update_row("testtable", row2id, updaterow.clone())
            .await?;
